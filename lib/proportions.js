define(["chroma-js", "virtual-dom", "numeral-intl", "filters/genericnode", "vercomp" ],
  ...
  return function (config, filterManager) {
    ...
    roleTable.classList.add("proportion")

    var gluonVersionTable = document.createElement("table")
    gluonVersionTable.classList.add("proportion")
    ...
    self.setData = function (data) {
      ...
      var roleDict = count(nodes, ["nodeinfo", "system", "role"], function (d) {
        var rt = d
        if (config.role)
          config.role.forEach( function (t) {
            if(d === t.role)
              rt = t.name
          })
        return rt
      ...
      var gluonVersionDict = count(nodes, ["nodeinfo", "software", "firmware", "base"], function (d) {
        var rt = d
        if (config.gluonVersion)
          config.gluonVersion.forEach( function (t) {
            if(d === t.gluonVersion)
              rt = t.name
          })
        return rt
      ...
      fillTable("Rolle", roleTable, roleDict.sort(function (a, b) { return b[1] - a[1] }))
      fillTable("Gluon Version", gluonVersionTable, gluonVersionDict.sort(function (a, b) { return vercomp(b[1], a[1]) }))
    ...
    self.render = function (el) {
      ...
      self.renderSingle(el, "Gluon Version", gluonVersionTable)
